...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString: String, success:@escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(url, parameters: ["status":tweetString], progress: nil, success: {(tasks: URLSessionDataTask, response: Any?) in success()
    ...
    func favoriteTweet(tweetID: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetID], progress: nil, success: {(tasks: URLSessionDataTask, response: Any?) in success()
